	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			DataSource ds = (DataSource) context
					.lookup("java:comp/env/jdbc/WIME");
			conn = ds.getConnection();
			PreparedStatement pstmt = null;
			//System.out.println("GetImageFromDB, Type==>" + type);
			//System.out.println("GetImageFromDB, ID==>" + id);
			if (type.equalsIgnoreCase("product")) {  // 讀取eBook表格
				pstmt = conn.prepareStatement(
						"SELECT pd_pic, pd_image from product where pd_ID = ?");
			...
			if (rs.next()) {
				...
				String mimeType = getServletContext().getMimeType(picture);
				// 設定輸出資料的型態
